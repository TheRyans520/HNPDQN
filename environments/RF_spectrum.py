...
class RfEnvironment(gym.Env):
    ...
    def __init__(self, jammers, jammer_dist, jamming_power, csc, channels, length, stride, n_scans=10):
        ...
        self.stat_features = ['mean', 'std']
        # 每信道提取: snr_mean, snr_std, rssi_mean, rssi_std, noise_mean
        self.state_features = ['snr_mean', 'snr_std', 'rssi_mean', 'rssi_std', 'noise_mean']

        self.average_rssi = 0

        self.action_space = spaces.Discrete(len(self.channels))

        self.observation_size = len(self.channels) * len(self.state_features)
    ...
    def get_state(self, processed_channel_data):
        num_feats = len(self.state_features)
        state = np.zeros((len(self.channels), num_feats))
    ...
    def construct_state(self, processed_channel_data):
        ...
        # 多特征多统计量：snr, rssi, noise 的均值和标准差
        snr_mean = df_channel['snr'].mean()
        snr_std = df_channel['snr'].std()
        rssi_mean = df_channel['rssi'].mean()
        rssi_std = df_channel['rssi'].std()
        noise_mean = df_channel['noise'].mean()
        return np.array([snr_mean, snr_std, rssi_mean, rssi_std, noise_mean])
